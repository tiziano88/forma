...
class MessageValueImpl implements MessageValue {
  ...
  toObject(): Record<string, unknown> {
    ...
    for (const [fieldNumber, values] of this.fields as Iterable<
      [number, InterpretedValue[]]
    >) {
      const fieldDef = this.type.fields.get(fieldNumber);
      if (!fieldDef || values.length === 0) continue;

      if (fieldDef.label === FieldLabel.LABEL_REPEATED) {
        result[fieldDef.name] = values.map((value: InterpretedValue) =>
          this.valueToObject(value)
        );
      } else {
  ...
  toBytes(): Uint8Array {
    ...
    for (const fieldNumber of sortedFields) {
      const fieldDef = this.type.fields.get(fieldNumber);
      const values = this.fields.get(fieldNumber) as
        | InterpretedValue[]
        | undefined;
    ...
    switch (fieldDef.type) {
      case FieldType.TYPE_STRING:
        if (typeof value !== 'string') {
          throw new Error(
            `Field ${fieldDef.name} expects string, got ${typeof value}`
          );
        }
        break;
      case FieldType.TYPE_BOOL:
        if (typeof value !== 'boolean') {
          throw new Error(
            `Field ${fieldDef.name} expects boolean, got ${typeof value}`
          );
        }
        break;
      ...
      case FieldType.TYPE_ENUM:
        if (typeof value !== 'number') {
          throw new Error(
            `Field ${fieldDef.name} expects number, got ${typeof value}`
          );
        }
        break;
      case FieldType.TYPE_BYTES:
        if (!(value instanceof Uint8Array)) {
          throw new Error(
            `Field ${fieldDef.name} expects Uint8Array, got ${typeof value}`
          );
        }
        break;
      case FieldType.TYPE_MESSAGE:
        if (
          ...
          throw new Error(
            `Field ${fieldDef.name} expects MessageValue, got ${typeof value}`
          );
        }
        break;
    }
...
export class StructuralEditor {
  ...
  public setSchema = async (protoText: string): Promise<void> => {
    throw new Error(
      'setSchema method not implemented. Use setSchemaDescriptor with compiled .desc files instead.'
    );
  };

  public setSchemaDescriptor = async (
  ...
  private loadSchemaFromDescriptor = async (
    ...
    console.log(
      '[Editor Core] Loading schema from binary descriptor, size:',
  ...
  private printTypeRegistry = (): void => {
    ...
    for (const [typeName, messageType] of this.typeRegistry) {
      ...
      for (const [fieldNumber, fieldDef] of messageType.fields) {
        ...
        const labelStr =
          fieldDef.label === FieldLabel.LABEL_REPEATED ? 'repeated ' : '';
        console.log(
          `    ${fieldNumber}: ${labelStr}${typeStr} ${fieldDef.name}`
        );
      }
    }
  ...
  private createDecodedView = (): void => {
    ...
    console.log(
      '  dataBytes:',
      this.dataBytes ? `${this.dataBytes.length} bytes` : 'null'
    );
    ...
    console.log(
      '  messageType found:',
      messageType ? messageType.fullName : 'null'
    );
  ...
  private parseMessage = (
    ...
    console.log(
      `[Editor Core] Parsing message of ${bytes.length} bytes for type ${messageType.fullName}`
    );
    ...
    while (reader.nextField()) {
      ...
      console.log(
        `[Editor Core] Found field ${fieldNumber}, fieldDef:`,
        fieldDef ? fieldDef.name : 'unknown'
      );
      ...
      console.log(
        `[Editor Core] Parsed field ${fieldNumber} (${fieldDef.name}):`,
        value
      );
    ...
    console.log(
      `[Editor Core] Finished parsing. Found ${fieldCount} fields total.`
    );
    return messageValue;
  };
