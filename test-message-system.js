...
async function testMessageSystem() {
  ...
  try {
    ...
    const schemaPath = path.join(__dirname, 'vscode-extension/media/schemas/config.desc');
    ...
    console.log(`📄 Loading data (${dataBytes.length} bytes) and schema (${schemaBytes.length} bytes)...\n`);
    ...
    if (messageValue) {
      ...
      for (const fieldNumber of messageValue.getSetFields()) {
        ...
        if (fieldDef) {
          console.log(`  Field ${fieldNumber} (${fieldDef.name}):`, 
