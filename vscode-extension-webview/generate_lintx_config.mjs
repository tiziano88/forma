  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a.startsWith('--')) {
      ...
      if (args[key] !== undefined) {
        ...
        if (Array.isArray(args[key])) args[key].push(val); else args[key] = [args[key], val];
      } else {
        args[key] = val;
      }
    }
...
async function main() {
  ...
  const maps = [];
  const mapArgs = Array.isArray(args.map) ? args.map : args.map ? [args.map] : [];
  ...
  const files = maps.length
    ? maps.map(m => File.create({ data: m.data, schema: m.schema, type: m.type }))
    : [File.create({ data: args.data ?? 'structural-editor/public/sample.bin', schema: args.schema ?? 'structural-editor/public/sample.proto', type: args.type ?? 'example.Person' })];
