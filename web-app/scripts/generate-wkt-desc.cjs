...
const protoPaths = PROTO_FILES.map(file => path.join(WELL_KNOWN_TYPES_DIR, file)).join(' ');

const protoFiles = PROTO_FILES.map(file => path.join('google', 'protobuf', file)).join(' ');
