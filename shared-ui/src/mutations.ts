...
export class MutationUtils {
   ...
   */
  static createRepeatedPath(fieldNumbers: number[], finalIndex: number): FieldPath {
    const segments = fieldNumbers.map(fieldNumber => ({ fieldNumber, index: 0 }));
   ...
   */
  static createDescription(mutation: Omit<FieldMutation, 'id' | 'timestamp' | 'description'>): string {
    ...
    switch (mutation.type) {
      ...
      case 'ADD_REPEATED_FIELD':
        const indexStr = mutation.insertIndex !== undefined ? ` at index ${mutation.insertIndex}` : ' at end';
...
export class MutationApplicator {
   ...
   */
  static applyMutation(rootMessage: MessageValue, mutation: FieldMutation): void {
    ...
    try {
      switch (mutation.type) {
        ...
        case 'MOVE_REPEATED_FIELD':
          ...
          break;
        default:
          console.error('[MutationApplicator] Unknown mutation type:', (mutation as any).type);
      }
    } catch (error) {
      console.error('[MutationApplicator] Error applying mutation:', error, mutation);
    }
  }
   ...
   */
  private static navigateToField(rootMessage: MessageValue, path: FieldPath): {
    parent: MessageValue;
    fieldNumber: number;
    index: number;
  } {
    ...
    for (let i = 0; i < path.length - 1; i++) {
      ...
      if (segment.index === 0) {
        ...
        // Repeated field item
        const items = current.getRepeatedField(segment.fieldNumber) as MessageValue[];
        current = items[segment.index];
      }

      if (!current) {
        throw new Error(`Navigation failed at path segment ${i}: field ${segment.fieldNumber}, index ${segment.index}`);
      }
    }
  ...
  private static applySetField(rootMessage: MessageValue, mutation: SetFieldMutation): void {
    const { parent, fieldNumber, index } = this.navigateToField(rootMessage, mutation.path);
  ...
  private static applyClearField(rootMessage: MessageValue, mutation: ClearFieldMutation): void {
    const { parent, fieldNumber } = this.navigateToField(rootMessage, mutation.path);
    parent.clearField(fieldNumber);
  }

  private static applyAddRepeatedField(rootMessage: MessageValue, mutation: AddRepeatedFieldMutation): void {
    const { parent, fieldNumber } = this.navigateToField(rootMessage, mutation.path);
  ...
  private static applyRemoveRepeatedField(rootMessage: MessageValue, mutation: RemoveRepeatedFieldMutation): void {
    const { parent, fieldNumber } = this.navigateToField(rootMessage, mutation.path);
  ...
  private static applyMoveRepeatedField(rootMessage: MessageValue, mutation: MoveRepeatedFieldMutation): void {
    const { parent, fieldNumber } = this.navigateToField(rootMessage, mutation.path);

    const items = parent.getRepeatedField(fieldNumber) || [];
    if (mutation.fromIndex < 0 || mutation.fromIndex >= items.length ||
        mutation.toIndex < 0 || mutation.toIndex >= items.length) {
      throw new Error(`Invalid move indices: ${mutation.fromIndex} -> ${mutation.toIndex}, array length: ${items.length}`);
    }

    const newItems = [...items];
    [newItems[mutation.fromIndex], newItems[mutation.toIndex]] = [newItems[mutation.toIndex], newItems[mutation.fromIndex]];
    parent.clearField(fieldNumber);
    newItems.forEach(item => parent.addRepeatedField(fieldNumber, item));
  }
}
...
export class MutationDispatcher {
   ...
   */
  addRepeatedField(fieldNumber: number, value: FieldValue, insertIndex?: number): void {
   ...
   */
  moveRepeatedField(fieldNumber: number, fromIndex: number, toIndex: number): void {
